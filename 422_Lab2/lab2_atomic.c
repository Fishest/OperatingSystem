...
mark_nonprimes(int *ptr)
{
        while (1) {
                ...
                while (curr <= upper_bound && atomic_read(&arr[curr]) != curr) {
                        curr++;
                }
                if (curr*curr > upper_bound) {
                        spin_unlock_irqrestore(&cur_lock, cur_lock_macro);
                        return;
                }
                unsigned long prime = curr;
                curr++;                  spin_unlock_irqrestore(&cur_lock, cur_lock_macro);
...
simple_init(void)
{
        ...
        arr = kmalloc((upper_bound+1)*sizeof(atomic_t), GFP_KERNEL);
        if (arr == NULL) {
                printk(KERN_ERR "kmalloc failure!\n");
                kfree(counter);
                counter = NULL;
...
simple_exit(void)
{
        ...
        if (counter == NULL && arr == NULL) {
                ...
                printk(KERN_INFO "All the primes\n");
                for (; i<=upper_bound; i++) {

                        if (atomic_read(&arr[i]) == i) {
                                prime_count++;
                                //printk(KERN_CONT "%d", atomic_read(&arr[i]));
                                if (prime_count %8 == 0) {
                                        //printk(KERN_CONT "\n");
                                }
                        }
